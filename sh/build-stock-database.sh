...
echo "[build]"
echo "Building OpenSIGL Stock Database"
...
mysql -u "$DB_USER" -p"$DB_PASS" -h"$DB_HOST" -P"$DB_PORT" "$DB_NAME" < test/data/accounts.sql || { echo 'failed to import test data into DB 2/5' ; exit 1; }
mysql -u "$DB_USER" -p"$DB_PASS" -h"$DB_HOST" -P"$DB_PORT" "$DB_NAME" < test/data/inventories.sql || { echo 'failed to import test data into DB 3/5' ; exit 1; }
mysql -u "$DB_USER" -p"$DB_PASS" -h"$DB_HOST" -P"$DB_PORT" "$DB_NAME" < test/data/core.sql || { echo 'failed to import test data into DB 4/5' ; exit 1; }
mysql -u "$DB_USER" -p"$DB_PASS" -h"$DB_HOST" -P"$DB_PORT" "$DB_NAME" < test/data/service-stock.sql || { echo 'failed to import test data into DB 5/5' ; exit 1; }
