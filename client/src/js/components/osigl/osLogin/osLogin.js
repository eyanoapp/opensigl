angular.module('opensigl.components').component('osLogin', {
  templateUrl : 'js/components/osigl/osLogin/osLogin.html',
  controller : OsLoginController,
  transclude : true,
  bindings : {},
});

OsLoginController.$inject = [
  'appcache',
  'SessionService',
  'LanguageService',
  'ProjectService',
...
function OsLoginController(
  ...
  vm.$onInit = function onInit() {
    // load the languages
    Languages.read().then(handleLanguages);

    // load project dependencies
    Projects.read().then(handleProjects);

    // load the helpdesk information
    Helpdesk.read().then(setHelpdeskInfo);
