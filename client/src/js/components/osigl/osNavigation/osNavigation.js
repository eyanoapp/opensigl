angular.module('opensigl.components')
  .component('osNavigation', {
    controller : OsNavigationController,
    templateUrl : 'js/components/osigl/osNavigation/osNavigation.html',
  });

OsNavigationController.$inject = [
 ...
 */
function OsNavigationController($location, $rootScope, Tree, AppCache, Notify, $transitions, $state) {
  const $ctrl = this;
  ...
  function calculateUnitIndex(units) {
    units.forEach(unit => {
      ...
      unit.isSettings = unit.name.indexOf('[SETTINGS]') >= 0;
